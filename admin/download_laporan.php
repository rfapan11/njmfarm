...
$ambil = $koneksi->query("SELECT * FROM pembelian pm LEFT JOIN pelanggan pl 
							...
							WHERE status_pembelian='$status' 
							AND tanggal_pembelian 
							BETWEEN '$tgl_mulai' 
							AND '$tgl_selesai' ");
...
$isi .= "<h5>".date("d F Y",strtotime($tgl_mulai))." Hingga ".date("d F Y",strtotime($tgl_selesai))."</h5>";
$isi .= "<table class='table table-bordered' border='1'>";
	...
	$isi.= "<tfoot>";
		$isi.= "<tr>";
			...
			$isi.= "<th colspan='1'>Rp.".number_format($total)."</th>";
			$isi.= "<th></th>";
		$isi.= "</tr>";
	$isi.= "</tfoot>";
$isi.= "</table>";
